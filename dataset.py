...
class RecDataset(Dataset):
    def __init__(self, args, user_seq, test_neg_items=None, data_type='train',
        ...
        # info-loss analysis: keep only the most recent `truncate_k` input items
        # (None = no truncation, original behaviour). Used at eval only.
        ...
        # PROPOSED: sequence augmentation — per-item co-occurrence+direction associates
        self.assoc = None
        if getattr(args, "seq_augment", False):
            from model.cooc_dir import build_augment_associates
            self.assoc = build_augment_associates(args.data_file, args.item_size,
                                                  topk=50, cap=args.max_seq_length)
    ...
    def __getitem__(self, index):
        ...
        # info-loss analysis: keep only the most recent k items (right side = newest).
        if self.truncate_k is not None:
            input_ids = input_ids[-self.truncate_k:]

        # PROPOSED: expand the input sequence with co-occurrence+direction associates.
        # k times: pick a random item x from the sequence, insert its top associate j
        # (j not already in the sequence; x->j forward direction) right after x.
        # train: exclude this instance's own answer (avoid copy-collapse); stochastic.
        # eval : deterministic (seeded by index); inserted items are masked from
        #        prediction in the trainer, so this only enriches CONTEXT.
        if self.assoc is not None and len(input_ids) > 0 and int(self.args.aug_k) > 0:
            K = int(self.args.aug_k)
            excl = set(input_ids)
            if self.data_type == 'train':
                excl.add(answer)
            if getattr(self.args, 'aug_anchor', 'random') == 'last':
                # anchor on the LAST (most recent) item: append its top-K co-occurrence+
                # direction associates right after it (x -> assoc). target most strongly
                # relates to the last item, so this maximizes surfacing.
                x = int(input_ids[-1]); ins = []
                for cand in self.assoc[x]:
                    if cand < 0:
                        break
                    c = int(cand)
                    if c not in excl and c not in ins:
                        ins.append(c)
                        if len(ins) >= K:
                            break
                input_ids = list(input_ids) + ins
            else:
                seq = list(input_ids)
                rng = np.random if self.data_type == 'train' else np.random.RandomState(index + 1)
                for _ in range(K):
                    if not seq:
                        break
                    x = int(seq[rng.randint(len(seq))])
                    j = -1
                    for cand in self.assoc[x]:
                        if cand < 0:
                            break
                        if int(cand) not in excl:
                            j = int(cand); break
                    if j < 0:
                        continue
                    seq.insert(seq.index(x) + 1, j)
                    excl.add(j)
                input_ids = seq
